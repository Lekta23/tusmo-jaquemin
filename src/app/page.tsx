  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-black p-4">
      ...
      <div className="flex space-x-2 mb-4">
        <span className="p-2 border border-gray-300 rounded text-white">
          {word[0]}
        </span>
      ...
      {error && <div id='error_label' className="mt-2 text-red-500">{error}</div>}
      <div className="mt-4">
        {history.map((pastGuess, index) => (
          <div
            id={"result_word_" + index}
            key={index}
          ...
          >
            {feedback[index].map((color, i) => (
              <span
                key={i}
                id={"result_letter_" + i}
                className={`w-10 h-10 text-center font-bold p-2 rounded ${color}`}
              >
                {pastGuess[i]}
              </span>
            ))}
